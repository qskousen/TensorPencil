...
fn txtGemm(be: *Backend, arena: std.mem.Allocator, y: DeviceBuffer, x: DeviceBuffer, m: usize, w: anytype, out: usize, in: usize, bias: []const f32) !void {
    ...
    // complete and is not.
    try lin_cuda.probeInput(be, x, m, w);
...
pub fn forward(model: *const DiT, be: *Backend, sess: *const Session, ws: *const Workspace, io: std.Io, gpa: std.mem.Allocator, out: []f32, x_lat: []const f32, sigma: f32, cancel: ?*std.atomic.Value(bool)) !void {
    ...
    try lin_cuda.probeInput(be, imgin_d, n_img, model.first.w);
    ...
    for (model.blocks, 0..) |blk, b| {
        ...
        try lin_cuda.prep(be, plan, t1_d, seq, F, &.{ blk.attn.wq, blk.attn.wk, blk.attn.wv, blk.attn.gate }, false);
        try lin_cuda.gemm(be, plan, q_d, t1_d, seq, blk.attn.wq, false);
        try lin_cuda.gemm(be, plan, k_d, t1_d, seq, blk.attn.wk, false);
        try lin_cuda.gemm(be, plan, v_d, t1_d, seq, blk.attn.wv, false);
        try lin_cuda.gemm(be, plan, g_d, t1_d, seq, blk.attn.gate, false);
        ...
        try lin_cuda.prep(be, plan, attn_d, seq, blk.attn.wo.cols, &.{blk.attn.wo}, false);
        try lin_cuda.gemm(be, plan, t1_d, attn_d, seq, blk.attn.wo, false);
        try be.gatedAdd(x_d, t1_d, mv_d, seq * F, F, mb + 2 * F);
        // f16 activation chain through the MLP, only on the cuBLASLt/irescale int8 libs
        ...
        while (c0 < seq) : (c0 += mlp_tile) {
            ...
            try lin_cuda.prep(be, plan, t1_d, tile, F, &.{ blk.mlp.gate, blk.mlp.up }, false);
            try lin_cuda.gemm(be, plan, mg_d, t1_d, tile, blk.mlp.gate, mlp_f16);
            try lin_cuda.gemm(be, plan, mu_d, t1_d, tile, blk.mlp.up, mlp_f16);
            if (mlp_f16) try be.siluMul16(mg_d, mu_d, tile * mlp_dim) else try be.siluMul(mg_d, mu_d, tile * mlp_dim);
            try lin_cuda.prep(be, plan, mg_d, tile, blk.mlp.down.cols, &.{blk.mlp.down}, mlp_f16);
    ...
    try lin_cuda.probeInput(be, offsetBuf(t1_d, seq_txt * F * 4), n_img, model.last_linear.w);
