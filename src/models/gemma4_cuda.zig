...
pub const CudaLM = struct {
    ...
    pub fn warmWeights(self: *CudaLM) void {
        ...
        for (self.lm.layers, 0..) |*layer, l| {
            ...
            // gemma4's `v` is optional (shared-KV layers carry none).
            if (layer.v) |vw| be.warmWeight(vw.bytes);
...
// once the session owner seeds it (engine/chat.zig Session.init does).
