//! NVIDIA's Philox4x32-10 + Box-Muller standard normals — the noise AUTOMATIC1111 draws
//! its initial latent from, reproduced on the CPU.
//!
//! **Why this exists alongside `torch_rng.zig`.** The two ecosystems draw their initial
//! noise from *different generators*, so the same seed is not the same picture:
//!
//! | | generator | this engine's default |
//! |---|---|---|
//! | ComfyUI | `torch.randn` on the **CPU** — MT19937 + AVX2 `normal_fill` | `torch_rng.zig` |
//! | A1111 (`randn_source="GPU"`, the default) | `torch.randn` on **CUDA** — Philox4x32-10 + Box-Muller | this file |
//!
//! It is not a small difference: a different algorithm gives an *unrelated* noise tensor,
//! not a slightly perturbed one. Upstream's own description of the option is "changes
//! seeds drastically". Every other A1111-vs-ComfyUI difference this codebase has recorded
//! is worth some number of dB; this one decides whether the image is the same image.
//!
//! **The reference is A1111's own `modules/rng_philox.py`** — 100 lines of pure numpy
//! titled "RNG imitiating torch cuda randn on CPU. You are welcome." — which is what its
//! third `randn_source` setting, "NV", selects. So the port target is executable, and
//! `tools/gen_philox_fixtures.py` runs it rather than re-deriving anything.
//!
//! ⚠️ **Two places where "imitating CUDA" is exact and one where it is not.** Both were
//! settled by reading the kernel that actually runs
//! (`ATen/native/cuda/DistributionTemplates.h` and curand's `curand_normal.h`), because
//! the mapping is not what the numpy code's shape suggests:
//!
//! - **The element→counter map is exact for ordinary tensors.** ATen's kernel gives each
//!   thread ONE `curand_normal4` call and spreads its four normals across a grid stride,
//!   so naively element `i` is not thread `i`'s first normal. But the launch uses
//!   `grid.x = min(SM_count · (maxThreadsPerSM / 256), ceil(numel/256))`, so as long as
//!   `numel <= 256 · grid_cap` there is one thread per element and only `rand.x` is ever
//!   consumed — which is exactly `counter[2] = i`, sin only. ATen's own comment says so:
//!   "for common tensor sizes, we end up using rand.x from each thread."
//! - ⚠️ **Above that cap it stops being exact, and the cap is HARDWARE-DEPENDENT.** On a
//!   3090 (82 SMs × 1536 threads/SM ÷ 256) it is 492 blocks = **125,952 elements** — above
//!   a 1024×1536 latent's 98,304 but below a 1536² latent's 147,456. Past it the tail of
//!   the tensor comes from the `rand.y/z/w` slots (the *cosine* partners), so the noise
//!   genuinely differs. `Generator.threads` reproduces that regime; nothing in the CLI
//!   sets it, because reproducing a *foreign* A1111 render would require knowing which GPU
//!   drew it. A1111's own "NV" setting has the same limitation — it is the uncapped form.
//! - **The arithmetic is numpy's f64, not curand's f32.** `uint32 * float32` promotes to
//!   **float64** under NumPy's rules, so `box_muller` runs in f64 with f32-rounded
//!   constants and narrows once; curand's device path is f32 throughout and uses the
//!   `__sincosf` intrinsic. Matching numpy makes this bit-exact against `randn_source="NV"`
//!   and ~1e-6 relative against `"GPU"` — two orders below the 1.3e-4 model-level
//!   disagreement the render comparison already carries.
//!
//! Pinned by `src/core/assets/philox_fixtures.json` (the block cipher and the normals
//! separately, so a mismatch localizes).
...
/// unrolled slots — see the module doc).
...
pub const Generator = struct {
    ...
    offset: u32 = 0,
    /// How many CUDA threads the launch being reproduced used, or **0 for one counter
    /// per element** — A1111's "NV" path and the right answer for any tensor below the
    /// launch's thread cap. See the module doc for when this is not the same thing.
    threads: u32 = 0,
