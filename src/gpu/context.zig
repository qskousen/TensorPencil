...
pub const Context = struct {
    ...
    /// Move each row of `x` along a unit direction (`dit.ActSteer`). a = rows,
    /// c = directions; `dir_off` is where this block's direction starts.
    pub fn opActSteer(
        self: *Context,
        x: DeviceBuffer,
        dirs: DeviceBuffer,
        ...
        try self.opElt(.act_steer, x, null, dirs, null, .{
            .u0 = @intCast(rows),
            .u1 = @intCast(dim),
            .u2 = @intFromBool(add_op),
            .u3 = @intFromBool(keep_norm),
            .u4 = @intCast(dir_off),
            .u5 = @intCast(row_off),
            .f0 = scale,
        }, rows, 1, 1);
